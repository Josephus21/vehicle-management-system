...
export default function EditModal({ selectedRow, onDrawerClose, drawerVisible, onRefresh }) {
  ...
  const [selectedAracDetay, setSelectedAracDetay] = useState(null);
  const showModal = () => {
    setOpen(true);
  };
  ...
  const handleDataFetchAndUpdate = async () => {
    ...
    try {
      ...
      const aracDetay = await AxiosInstance.get(`Vehicle/GetVehicleById?id=${selectedRow.aracId}`);
      setSelectedAracDetay(aracDetay.data);
  ...
  const onRefresh1 = (selectedAxleId) => {
    if (selectedAxleId) {
      // Update the selectedRow with the new aksId
      selectedRow.aksId = selectedAxleId;
    }
    handleDataFetchAndUpdate();
  };
          ...
          {loading ? (
            ...
            <form onSubmit={methods.handleSubmit(onSubmit)}>
              <div style={{ padding: "5px" }}>
                {selectedRow?.aksId > 0 ? (
                  <Tablar selectedRow={selectedRow} onRefresh1={onRefresh1} selectedAracDetay={selectedAracDetay} />
                ) : (
                  <MainTabs selectedRow={selectedRow} onRefresh1={onRefresh1} selectedAracDetay={selectedAracDetay} />
                )}
              </div>
            </form>
          )}
